...
subtitle: "Which factors would make a World Cup player?"
team: wattle
author:
  - Ketan Kabu  31554679
...
squads_cleaned <- squads_cleaned %>%
  mutate(year = 2019) 
names(wc_squad_full)[1] <- "squad_no"
squads_cleaned <- squads_cleaned %>%
  rbind(wc_squad_full)
...
After we joined the two data sets, once again we checked the missing values for this newly created "squads_cleaned"

```{r miss-data,fig.cap="Missing data for squads"}
visdat::vis_miss(squads_cleaned)
```

The information of caps, goals and clubs were mostly not available for the World Cup that took place too long ago. In each analysis, we will filter only the years that have suffice information that can respond to the tasks on hand.  
...
- Data description: We obtained more data, so we did further introduction. Also the missing values analysis was conducted on both the new and old data sets.  
...
- Part 3.3 _Performance of successful clubs in the 2019 World Cup_: we nested the analysis of goals scored by countries under the analysis of goals and expand the analysis further to determine the relationship between the goals and the winning chance. The World Cup progress section was replaced by the overall summary of the progress for all nations in all World Cup tournaments.  
...
- Does the total number of goals have anything to do with the chance of winning the championship?

- Will the country with more goals win the game? 
...
Looking into figure \@ref(fig:score-dis), the USA and Germany score significantly more goals each year than other countries, the kurtosis reflects the central tendency. The more right the kurtosis, the more goals are scored. The point shows the total number of goals scored by different countries each year. It can be seen that the most was scored by the United State team. 
...
```{r exp-layout, fig.width = 15, fig.cap = "country score layout"}
...
squads_cleaned %>% 
  ...
  summarise("Player" = n_distinct(player)) %>% 
  arrange(club, year) %>%
  ggplot(aes(x = factor(year), Player, fill = as.factor(year))) +
...
domestic %>%
  ...
  geom_col(stat = "identity",fill="#f68060", fill="#f68060", alpha=.6, width=.4) +
  coord_flip() +
  # theme_bw() + 
  ylab("club") +
...
squads_mean <-squads_cleaned %>% 
  group_by(year) %>% 
  summarise(age = mean(age))
  
ggplot()+
  geom_histogram(binwidth = 1 ,squads_cleaned, mapping = aes(age,fill= as.factor(year)), color = "black")+
  geom_vline(squads_mean, mapping = aes(xintercept = age), color = "red", size = 1.5)+
  geom_text(squads_mean, mapping = aes(x = age+3, y = 50, label = round(age,digits = 2)))+
  facet_wrap(year~.)+
  theme_minimal()+
  theme(legend.position = "none")+
  labs(y = "Count", title = "Player's age distribution")

```

We chose the binwidth equals to 1, meaning each binwidth represented 1 year of age. The red line indicated the average age of the players by year. We could ignore the count for each year as the number of teams participated were not similar for the all the years. Instead we will concentrate on the distribution. 
