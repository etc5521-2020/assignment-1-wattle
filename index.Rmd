...
### Player's age
First of all, we will comb through the age of all participants that took part in World Cup from the very first tournament in 1991 to the most recent in 2019.  

```{r country-age, fig.cap = "Summary of player age by country"}
player_age <- squads %>%
  group_by(year, country) %>%
  ggplot(aes(reorder(country, age, FUN = median), age)) +
  geom_boxplot() +
  theme_bw() +
...
On average, a World Cup player would not be younger than her 20s. The ideal age range for players would be around 25 to 27 years. The World Cup 
